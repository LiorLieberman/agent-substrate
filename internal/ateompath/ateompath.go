...
var (
	...
	SnapshotCacheDir = filepath.Join(BasePath, "snapshot-cache")
	...
	// WorkerCapacitySocket is the node-local atelet socket used by ateom
	// to report its worker capacity to the control plane.
	WorkerCapacitySocket = filepath.Join(BasePath, "worker-capacity.sock")
)
