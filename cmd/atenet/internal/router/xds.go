...
type XdsServer struct {
	...
	// Upstream (actor-facing) mTLS. When upstreamClientCertPath is set, the
	...
	// client cert and validates the atunnel server against upstreamTrustPath.
...
func (x *XdsServer) SetUpstreamTls(clientCertPath, trustPath, spiffePrefix string) {
	x.mu.Lock()
	defer x.mu.Unlock()
	x.upstreamClientCertPath = clientCertPath
	x.upstreamTrustPath = trustPath
...
func (x *XdsServer) buildUpstreamTransportSocket() *corev3.TransportSocket {
		...
		TlsCertificates: []*tlsv3.TlsCertificate{
			{
				CertificateChain: &corev3.DataSource{
					Specifier: &corev3.DataSource_Filename{Filename: x.upstreamClientCertPath},
				},
	...
	if x.upstreamTrustPath != "" {
		validationCtx := &tlsv3.CertificateValidationContext{
			TrustedCa: &corev3.DataSource{
				Specifier: &corev3.DataSource_Filename{Filename: x.upstreamTrustPath},
			},
		}
...
func (x *XdsServer) buildOriginalDstCluster() *clusterv3.Cluster {
	...
	if ts := x.buildUpstreamTransportSocket(); ts != nil {
		...
		cluster.TypedExtensionProtocolOptions = map[string]*anypb.Any{
			"envoy.extensions.upstreams.http.v3.HttpProtocolOptions": httpOpts,
		}
	}
